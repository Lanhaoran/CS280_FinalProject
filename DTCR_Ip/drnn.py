...
def dRNN(cell, inputs, rate, scope='default'):
    ...
    """
    b, d = tf.unstack(tf.shape(inputs[0])) # b -> batch_size; d -> dimension
    ...
    dilated_outputs, _ = tf.nn.static_rnn(#output shape ?? list, len = dialated_n_steps, each of them is [b*rate, hidden_units]
        ...
        dilated_inputs,
        dtype=tf.float32, 
        scope=scope)#dilated_states shape [b*rate, hidden_unit] hidden_unit 应该在cell实例里面声明的
...
def multi_dRNN_with_dilations(cells, inputs, dilations, scope='drnn'):
    ...
    final_state_list = []
    for cell, dilation in zip(cells, dilations):   #cell与对应的dilation做一个压缩封装
