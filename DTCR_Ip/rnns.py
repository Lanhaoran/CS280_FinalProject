...
class dilated_encoder():
    ...
    def encoder(self, inputs):
        ...
        cell_fw_list = [tf.nn.rnn_cell.GRUCell(num_units=units, 
        ...
        cell_bw_list = [tf.nn.rnn_cell.GRUCell(num_units=units, 
                                  kernel_initializer=tf.random_normal_initializer(stddev=0.01),
                                  bias_initializer=tf.zeros_initializer()) 
                                  for units in self.hidden_units]
        '''
...
class single_layer_decoder():
    ...
    def decoder(self, init_state, init_input):
        ...
        cell = tf.nn.rnn_cell.GRUCell(num_units = self.hidden_units, 
                                  kernel_initializer=tf.random_normal_initializer(stddev=0.01),
                                  bias_initializer=tf.zeros_initializer())
        '''
        cell = tf.nn.rnn_cell.GRUCell(num_units = self.hidden_units)
        ...
        # recons = outputs[:, :, :96]            # multi dimension
        recons = outputs[:, :, :config_dtcr['feature_num']]           
